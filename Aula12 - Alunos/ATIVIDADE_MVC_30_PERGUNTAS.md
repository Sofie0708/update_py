...
Ficam na pasta models/, flask/Aula12/models/filme_favorito.py e flask/Aula12/models/historico_busca.py.

**2.** Qual é o nome do arquivo de banco criado quando o app roda? Em qual arquivo Python essa configuração está?
O nome do arquivo é streamflix.db

**3.** Quais classes Model existem no projeto (nome das classes)? Em quais arquivos `.py` cada uma está?
Classe ModeloBase — no arquivo base.py
Classe FilmeFavorito — no arquivo filme_favorito.py
Classe HistoricoBusca — no arquivo historico_busca.py

**4.** De qual superclasse `FilmeFavorito` e `HistoricoBusca` herdam? O que elas ganham automaticamente por herança (cite 3 campos)?
Elas herdam da superclasse ModeloBase. Por herança, ganham automaticamente id (Chave primária inteira), data_criacao (Data e hora do registro) e data_atualizacao (Data e hora da última modificação).

**5.** Qual é o `__tablename__` da tabela de favoritos? Por que usamos `__tablename__` em vez de só o nome da classe?
O __tablename__ é "filmes_favoritos". Usamos essa propriedade para definir o nome real da tabela física no banco de dados SQLite utilizando o padrão snake_case, em vez de deixar o SQLAlchemy gerar automaticamente.
...
A coluna é tmdb_id. Suas restrições especiais são: nullable=False e unique=True.
...
Ele primeiro chama o método cls.buscar_por_tmdb(tmdb_id) para verificar se o filme já está cadastrado.
Se o filme já existir, o método retorna None imediatamente e interrompe a execução, impedindo a duplicação.
Se não existir, ele cria uma nova instância da classe FilmeFavorito preenchendo os parâmetros recebidos (tmdb_id, titulo, poster_path, nota, ano).
Adiciona o objeto à sessão do banco de dados via db.session.add(fav).
Confirma a transação com db.session.commit() e retorna o objeto criado.

**8.** Onde está o método que lista as últimas 8 buscas? Qual é o nome da classe e do método?
Está no arquivo models/historico_busca.py. O nome da classe é HistoricoBusca e o método é o ultimas(cls, limite=8).
